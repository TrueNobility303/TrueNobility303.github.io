...
上述构造在原文的NIPS会议版本上给出，但笔者发现上述函数的强凸系数为 $1/d$。那么上述下界是否仅能说明关于参数 $\epsilon$ 的复杂度是紧的，但是关于强凸系数或者维度大小 $d$ 至少有一个不紧呢？而笔者在arXiv上发现，作者在arXiv的最新版本上实际上采用了一个不同的构造，看起来用了一个较为复杂的构造和分析才得到最优的复杂度下界。出于时间关系，笔者在撰写的时候并没有额外的时间仔细阅读他们后续arXiv上的文章。
