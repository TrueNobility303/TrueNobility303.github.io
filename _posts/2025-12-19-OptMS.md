...
算法看起来有点复杂。我们进行一些简单的解释。框架上，算法大体上还是与 [外梯度法](https://truenobility303.github.io/EG/) 相似，用张量步更新变量原变量 $z$ 后，再紧接着更新对偶变量 $v$. 算法中的 $A_t = \sum_{i=0}^t a_t$ 也与之前加速算法中的量相同，我们希望证明算法的收敛率为 $\mathcal{O}(1/A_T)$ 并且希望 $A_t$ 是一个增长尽可能快的级数。$a'_{t+1}$ 的公式看着有点古怪，事实上它是一个二次函数的根，来源于 [Nesterov加速梯度法](https://truenobility303.github.io/Nesterov-Acceleration/) 类似的步骤，我们很快会在后续的证明中看到这个公式的作用。算法中的 $\lambda'_{t+1}$ 会是一个动态选择的期望步长大小的倒数，而 $\lambda_{t+1}$ 则代表实际张量步得到的步长大小的倒数。当实际量 $\lambda_{t+1} \ge \lambda'_{t+1}$ 的时候，我们需要用系数 $\gamma_{t+1} = \lambda'_{t+1} / \lambda_{t+1}$ 减缓对偶变量的更新幅度。
