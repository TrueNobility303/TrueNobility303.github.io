...
PPO的另一个启发式（但实际也经常被使用）的版本直接使用如下的梯度估计进行更新
...
上式的含义是，当优势函数 $A_{\phi'}(s_t,a_t)$ 为正值的时候，鼓励 $p_\phi(a_t \vert s_t)$ 增大，但为了稳定性，增大的幅度不能超过 $p_{\phi'}(a_t \vert s_t)$ 的 $1+\epsilon$ 倍。而当 $A_{\phi'}(s_t,a_t)$ 为负值的时候，鼓励 $p_\phi(a_t \vert s_t)$ 减小，但其不能超过 $p_{\phi'}(a _t \vert s_t)$ 的 $1-\epsilon$ 倍。PPO利用上面的目标函数，权衡了优化目标和重要性采样中的分布限制。
